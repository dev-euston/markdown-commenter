...
Open or drag-and-drop to load each one; download to export your comments. Documents render with **GitHub-flavored Markdown** (tables, task lists, strikethrough, autolinks).
...
- **`Open .zip`** loads a single archive holding one Markdown file and one comments JSON at its root.
- **`Download .zip`** exports the document and its comments together as one file.
- Packing and unpacking happen in-browser with `fflate` — still nothing is uploaded.

### Guided onboarding

- A **three-step tour** (load → comment → download) auto-launches on a first visit and spotlights each control it describes.
- Reopen it any time via the **`? Help`** button; once dismissed, it stays out of the way.
